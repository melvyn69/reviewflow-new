...
export const api = {
  ...
  competitors: {
      ...
      add: async (data: any) => {
          if (isSupabaseConfigured()) {
             const user = await authService.getUser();
             if (user?.organization_id) {
                 const { data: comp } = await supabase!.from('competitors').insert({...data, organization_id: user.organization_id}).select().single();
                 return comp;
             }
          }
          return INITIAL_COMPETITORS[0];
      }
  },
  ...
  locations: { 
      create: async (data: any) => {
          if (isSupabaseConfigured()) {
              const user = await authService.getUser();
              if (user?.organization_id) {
                  const { data: loc, error } = await supabase!.from('locations').insert({...data, organization_id: user.organization_id, connection_status: 'disconnected'}).select().single();
                  if (error) throw error;
                  return loc;
              }
          }
  ...
  billing: {
      ...
      createCheckoutSession: async (plan: string) => { 
          ...
          return STRIPE_LINKS[plan] || "https://stripe.com"; 
      },

...
const RouterContext = React.createContext<{ path: string; navigate: (p: string, opts?: {replace?: boolean}) => void; params: Record<string, string> }>({ 
    path: '/', 
    navigate: () => {},
    params: {} 
});
...
export const Routes: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return <>{React.Children.map(children, child => {
        ...
        // Ensure path is a string to avoid runtime errors on methods like endsWith
        if (typeof path !== 'string') return null;

        if (path === currentPath) match = true;
        else if (path === '*' || path === '/*') match = true;
        else if (path.endsWith('/*')) {
             const base = path.slice(0, -2);
             if (currentPath.startsWith(base)) match = true;
        }
        ...
        if (match) {
            ...
            if (Object.keys(params).length > 0) {
                return (
                    <RouterContext.Provider value={{ ...useContext(RouterContext), params }}>
                        {child}
                    </RouterContext.Provider>
                );
            }
...
export const useLocation = () => {
    ...
    return { pathname, search: search ? `?${search}` : '', hash: window.location.hash, state: null, key: 'default' };
};
